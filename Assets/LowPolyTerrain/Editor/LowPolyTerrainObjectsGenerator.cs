...
public class LowPolyTerrainObjectsGenerator
{
	...
	public void GenerateObjects()
	{
		...
		var terrainGenerator = new LowPolyTerrainGenerator(terrain);
		var heightmap = terrainGenerator.GenerateHeightmap();
		...
		for (int x = 0; x < _Objects.ObjectPlacementMap.width; ++x)
		{
			for (int y = 0; y < _Objects.ObjectPlacementMap.height; ++y)
			{
				if (Random.Range(0.0f, 1.0f) < _Objects.ObjectPlacementMap.GetPixel(x, y).grayscale * _Objects.ObjectProbabilityScale)
				{
					...
					// Grab the height
					float height = heightmap.SampleNormalizedHeight(uv) * terrain.TerrainHeight;
					Vector3 objectPosition = new Vector3(uv.x * terrain.TerrainSize, height - _Objects.ObjectHeightOffset, uv.y * terrain.TerrainSize);
